...
Matrix::Matrix(int row, int column, std::vector<std::vector<double> > data) {
	this->row = row;
	this->column = column;
	for(int i = 0; i < row; i++) {
...
Matrix operator*(const double & a, const Matrix & other) {
...
ostream& operator<<(ostream& os,const Matrix& it){
